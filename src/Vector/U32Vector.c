...
struct _ADT_U32Vector {
	size_t capacity, length;
	uint32_t *data;
};

static inline void U32Vector_realloc(U32Vector self, size_t new_capacity);
static inline void U32Vector_init(U32Vector self, size_t capacity);
static inline void U32Vector_final(U32Vector self);


U32Vector U32Vector_new()
{
	return U32Vector_newWithCapacity(1);
}

U32Vector U32Vector_newWithCapacity(size_t capacity)
{
	U32Vector self = malloc(sizeof(*self));
	U32Vector_init(self, capacity);
	return self;
}

U32Vector U32Vector_newFromArray(uint32_t const *array, size_t length)
{
	U32Vector self = U32Vector_newWithCapacity(length);
	memcpy(self->data, array, sizeof(uint32_t) * length);
	self->length = length;
	return self;
}
...
{
	if (src == NULL) return U32Vector_new();
	return U32Vector_newFromArray(src->data, src->length);
}

void U32Vector_copy(U32Vector self, U32Vector src)
{
	if (self == src) return;
	U32Vector_copyArray(self, src->data, src->length);
}

void U32Vector_copyArray(U32Vector self, uint32_t const *array, size_t n)
{
	U32Vector_final(self);
	U32Vector_init(self, n);
	memcpy(self->data, array, sizeof(uint32_t) * n);
	self->length = n;
}
...
void U32Vector_copySub(U32Vector self, U32Vector src, size_t start, size_t n)
{
	if (self != src && start < src->length) {
		...
		U32Vector_copyArray(self, src->data + start, n);
	}
}
...
void U32Vector_setCapacity(U32Vector self, size_t capacity)
{
	...
	U32Vector_realloc(self, capacity);
}

void U32Vector_shrink(U32Vector self)
{
	U32Vector_setCapacity(self, self->length);
}
...
#define Vector_preInsert(self, index) \
	...
	if (self->length >= self->capacity) { \
		U32Vector_realloc(self, self->capacity * 2); \
	} \
...
{
	return U32Vector_appendArray(self, src->data, src->length);
}
...
size_t U32Vector_appendArray(U32Vector self, const uint32_t *a, size_t count)
{
	...
	if (self->length + count > self->capacity) {
		U32Vector_realloc(self, self->capacity + count);
	}
	memcpy(self->data + self->length, a, sizeof(uint32_t) * count);
	self->length += count;
	return self->length;
}
...
static inline void U32Vector_realloc(U32Vector self, size_t new_capacity)
{
	uint32_t *tmp = calloc(new_capacity, sizeof(uint32_t));
	if (self->length > new_capacity) {
		self->length = new_capacity;
	}
	memcpy(tmp, self->data, sizeof(uint32_t) * self->length);
	free(self->data);
	self->data = tmp;
